...
int main(void){
	// Creating masterlist array variable
	masterlist dude[100];
	FILE *input;
	// Reading binary file to array variable
	input = fopen("output.bin","rb");
	// Counting number of items in masterlist
	...
	// Function that replaces the masterlist array with a new masterlist array of size n+1 with a new user specified element
	...
	// Function that prints the number of items in a specific category
	...
	// Function that prints all the items in the masterlist
	void print_list(masterlist a[], int c){
		for(int i =0; i<c; i++){
			printf("%s , $%d, aisle %d, category %c", a[i].item_name,a[i].cost,a[i].aisle, a[i].category);
		}
		
	}
	
	//Function that interacts with the user and fetches options and inputs to run the three functions above
	void UserInterface(masterlist a[], int c){
		while(option != 'x' && option != 'X'){
			...
			if (option == 'a' || option == 'A'){
				...
				bravo = addtomasterlist(a, alpha, number_of_items);
				a = bravo;
